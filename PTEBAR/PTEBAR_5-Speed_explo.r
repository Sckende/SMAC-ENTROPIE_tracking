...
lapply(speed.l, function(x){
  max(x$speed.km.h, na.rm = T)})

# outlier 1
out_1 <- speed.l[["PAC15-4"]]
hist(out_1$speed.km.h)
out_1[out_1$speed.km.h > 100,]

# outlier 2
out_2 <- speed.l[["PAC06-3"]]
hist(out_2$speed.km.h)
out_2[out_2$speed.km.h > 100,]

# Speed DF
speed.df <- do.call('rbind', speed.l)
tail(sort(speed.df$speed.km.h), 10)
summary(speed.df$speed.km.h)
hist(speed.df$speed.km.h)
