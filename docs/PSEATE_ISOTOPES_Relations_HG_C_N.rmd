...
Courbe rouge = regression linéaire  
Courbe bleue = GAM  
Si juste une courbe bleue, c'est que les deux relations se superposent.  
...
mod_gam <- mgcv::gam(iso$HG_MEAN ~ s(iso$CENTRO_LAT))
...
mod_gam <- mgcv::gam(iso$d13C_MEAN ~ s(iso$CENTRO_LAT))
pred_mod_gam <- predict(mod_gam,
                        se.fit = TRUE)
pred_df <- data.frame(x = iso$CENTRO_LAT,
                      fit_gam = pred_mod_gam$fit,
...
mod_gam <- mgcv::gam(iso$d15N_MEAN ~ s(iso$CENTRO_LAT))
pred_mod_gam <- predict(mod_gam,
                        se.fit = TRUE)
pred_df <- data.frame(x = iso$CENTRO_LAT,
                      fit_gam = pred_mod_gam$fit,
...
```{r, echo=FALSE}
           ...
           mod_quadra <- lm(iso$SST_MEAN ~ poly(iso$CENTRO_LAT, 2))
           
           mod_gam <- mgcv::gam(iso$SST_MEAN ~ s(iso$CENTRO_LAT))


# Visualization
pred_mod <- predict.lm(mod,
                    se.fit = TRUE)
# pred_mod_quadra <- predict.lm(mod_quadra,
#                               se.fit = TRUE)
pred_mod_gam <- predict(mod_gam,
                        se.fit = TRUE)
pred_df <- data.frame(lat = iso$CENTRO_LAT,
                      ...
                      # fit_sst_quadra = pred_mod_quadra$fit,
                      # se_fit_sst_quadra = pred_mod_quadra$se.fit,
...
####
           # lines(pred_df$lat,
           #       pred_df$fit_sst_quadra,
           #       col = "green")
           # lines(pred_df$lat,
           #       pred_df$fit_sst_quadra - pred_df$se_fit_sst_quadra,
           #       col = "green",
           #       lty = 4)
           # lines(pred_df$lat,
           #       pred_df$fit_sst_quadra + pred_df$se_fit_sst_quadra,
...
```{r, echo=FALSE}
           ...
           mod_gam <- mgcv::gam(iso$SST_MEAN ~ s(iso$HG_MEAN))
...
```{r, echo=FALSE}
           ...
           mod_gam <- mgcv::gam(iso$SST_MEAN ~ s(iso$d13C_MEAN))
...
```{r, echo=FALSE}
           ...
           mod_gam <- mgcv::gam(iso$SST_MEAN ~ s(iso$d15N_MEAN))
