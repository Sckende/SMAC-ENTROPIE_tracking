...
source('C:/Users/Etudiant/Desktop/SMAC/GITHUB/SMAC-ENTROPIE_tracking/PTEBAR_0-Functions_for_scripts.R')
...
```{r, include = F}
list_fix_freq <- lapply(gps_list2, fix_freq,
                        date_min = min(do.call('rbind', gps_list2)$time),
                        date_max = max(do.call('rbind', gps_list2)$time))
```

```{r, echo = F, message = F, warning = F}
l <- htmltools::tagList()
for(i in 1:length(list_fix_freq)){
  l[[i]] <- barp_fix_freq(list_fix_freq[[i]], use_NA = T)
}
l
```
